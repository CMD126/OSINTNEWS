...
def save_markdown(results: list, path: str, analysis=None) -> str:
    """
    Export results (and optional AI analysis) as a Markdown file.
    Suitable for Obsidian, Notion, GitHub, or any Markdown viewer.
    """
    cats: dict = {}
    for r in results:
    ...
    if analysis and analysis.succeeded:
        ...
        if analysis.cited_sources:
            lines += ["### Cited Sources", ""]
            for url in analysis.cited_sources:
                lines.append(f"- {url}")
            lines.append("")
        lines += ["---", ""]

    # News results grouped by category
    ...
    for cat, items in cats.items():
        ...
        for item in items:
            ...
            lines += [
                f"#### [{title}]({url})",
                f"",
                f"{snippet}{'…' if len(item.get('body', '') or '') > 200 else ''}",
                f"",
                f"> `{item.get('query', '')}`",
                "",
            ]

    lines += ["---", "", "*Generated by OSINTNEWS Plus — RAG Intelligence Engine*"]
